...
function GetSigImageB64(callback) {
    ...
    xhr2.onload = function() {
      ...
      img.onload = function() {
        ...
        // Obtener los datos de pÃ­xeles en formato de 24 bits
        var imageData = cntx.getImageData(0, 0, cvs.width, cvs.height);
        var data24 = new Uint8Array(imageData.width * imageData.height * 3);
        var k = 0;
        for (var i = 0; i < imageData.data.length; i += 4) {
          data24[k++] = imageData.data[i];
          data24[k++] = imageData.data[i + 1];
          data24[k++] = imageData.data[i + 2];
        }
  
        // Convertir los datos de pÃ­xeles de 24 bits a imagen PNG base64
        var imgData = new ImageData(data24, cvs.width, cvs.height);
        var tempCvs = document.createElement('canvas');
        tempCvs.width = cvs.width;
        tempCvs.height = cvs.height;
        var tempCntx = tempCvs.getContext('2d');
        tempCntx.putImageData(imgData, 0, 0);
        var b64String = tempCvs.toDataURL("image/jpeg");
        var loc = b64String.search("base64,");
        var retstring = b64String.slice(loc + 7, b64String.length);
        if (callback) {
          callback(retstring);
